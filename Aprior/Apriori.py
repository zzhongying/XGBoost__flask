...
if __name__ == '__main__':
    file_json = "class1.json"
    # 加载数据
    data_set = load_data_json(file_json)
    # 把数据转为数字 方便比较计算
    data_set, index_2_str = data_2_index(data_set)
    # 得到所有频繁项集
    L = get_all_L(data_set, 0.5)
    print("得到频繁项集")
    # 得到所有关联规则
    result = rules_from_L(L, min_confidence=0.85)
    print("得到关联规则1.。。。")
    ...
    for i in result:
        dic = {
            "feature_left": list(i["左"]),
            "feature_right": list(i["右"]),
            "support": i['支持度'],
            "confidence": i['置信度'],
            "lift": i['提升度']
        }
        apriori_list.append(dic)
    name = "apriori_class1"
    DIC = {"data": apriori_list}
    print("开始保存。。。。。")
    with open("F:\\实验室项目\\安卓恶意软件项目\\网络安全组 (2)\\Android_flask_0109\\Android_flask_12.23\\Aprior\\" + name + ".json", "w", encoding='utf-8') as f:
